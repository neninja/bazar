defmodule BazarWeb.Backoffice.UserLive.ConfirmationTest do
  ...
  describe "Confirm user" do
    test "renders confirmation page for unconfirmed user", %{conn: conn, unconfirmed_user: user} do
      ...
      {:ok, _lv, html} = live(conn, ~p"/backoffice/login/#{token}")
    ...
    test "confirms the given token once", %{conn: conn, unconfirmed_user: user} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/backoffice/login/#{token}")
      ...
      {:ok, _lv, html} =
        live(conn, ~p"/backoffice/login/#{token}")
    ...
    test "raises error for invalid token", %{conn: conn} do
      {:ok, _lv, html} =
        live(conn, ~p"/backoffice/login/invalid-token")
        |> follow_redirect(conn, ~p"/backoffice/login")
