defmodule BazarWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, BazarWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to settings when user is already logged in", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/backoffice/users/settings"
    end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      BazarWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_authenticated_user/2" do
    ...
    test "redirects if user is not authenticated", %{conn: conn} do
      ...
      assert redirected_to(conn) == ~p"/backoffice/users/log-in"
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        BazarWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
